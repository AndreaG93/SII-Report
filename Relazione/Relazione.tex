...
Lo scopo della presente relazione è quella di fornire un'analisi il più possibile dettagliata del malware denominato \textbf{FASTCash}\footnote{\url{https://www.symantec.com/security-center/writeup/2018-110615-2942-99}} la cui paternità, secondo le analisi forensi eseguite da varie aziende ed agenzie, tra cui la \textbf{Symantec}, \textbf{Kaspersky} e la \textbf{NCCIC}\footnote{Stiamo parlando della "\textit{National Cybersecurity and Communications Integration Center}" un'agenzia federale americana specializzata in sicurezza informatica facente parte della CISA (\textit{Cybersecurity and Infrastructure Security Agency}) \\ Per ulteriori informazioni in merito si veda: \url{https://www.us-cert.gov/about-us} }, è attribuita ad un gruppo di cyber-criminali conosciuto come \textit{Lazarus Group}\footnote{\url{https://www.symantec.com/blogs/threat-intelligence/fastcash-lazarus-atm-malware}} \footnote{\url{https://www.us-cert.gov/ncas/alerts/TA18-275A}}, di cui forniremo una descrizione più dettagliata nella sezione \ref{lazarausgrpo}.
...
Le analisi condotte dalla NCCIC hanno dimostrato che gli attaccanti, per mezzo del suddetto malware, sono riusciti a trasferire un \textbf{injection tool} all'interno dei server di payment switch delle banche coinvolte dell'attacco; analizzeremo in dettaglio l'injection tool di FASTCash nella sezione \ref{processoinjection}.

Il suddetto injection tool è stato utilizzato per introdurre, all'interno di uno o più processi legittimi in esecuzione sui payment switch server, codice malevolo capace di interagire ed alterare i messaggi basati sul \textbf{protocollo ISO 8583} scambiati tra gli ATM e il sistema informatico bancario centrale, consentendo agli aggressori di prelevare denaro agli sportelli automatici. 
...
Dal punto di vista strutturale, un messaggio ISO 8583 è composto da 3 campi:
...
Tale verifica avviene per mezzo di una funzione contenuta all'interno delle librerie introdotte nel sistema; la funzione \texttt{CheckPan()}\footnote{\url{https://security.arizona.edu/content/ta18-275a-hidden-cobra-–-fastcash-campaign}} \footnote{\url{https://www.symantec.com/blogs/threat-intelligence/fastcash-lazarus-atm-malware}}.
...
I rapporti pubblicati dalla NCCIC\footnote{\url{https://www.us-cert.gov/ncas/analysis-reports/AR18-275A}} \footnote{\url{https://www.us-cert.gov/ncas/alerts/TA18-275A}} e dalla Symantec\footnote{\url{https://www.symantec.com/blogs/threat-intelligence/fastcash-lazarus-atm-malware}} indicano che la forma di attacco adottata per compromettere i server dell'istituto bancario fosse stata una \textbf{process injection}.

Con la locuzione \textit{process injection} si intende una tecnica che rende possibile \textit{l'esecuzione di codice arbitrario precedentemente introdotto all'interno dello spazio d'indirizzamento di un processo distinto in esecuzione}.\footnote{\url{https://attack.mitre.org/techniques/T1055/}}
...
\item La riservatezza di tutte le transazioni, e quindi di tutti i dati ad essi associati, è stata compromessa poiché gli attaccanti sono riusciti ad accedere sia in lettura che in scrittura ai messaggi ISO 8583 scambiati tra i sistemi.
\end{itemize}
